...
/*
 * Initialise the mailbox for polled CPF communication used during
 * probe. Mailbox negotiation runs before adf_dev_up() so no MSI-X
 * vectors are allocated yet; ring-bundle vectors (and the mailbox
 * MSI-X vector at index 0) are allocated later by
 * adf_lce_isr_resource_alloc() via hw_data->alloc_irq.
 */
